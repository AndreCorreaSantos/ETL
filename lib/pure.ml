...
let group_by intermediate_results =

  let add_to_map key value map =
    match IntMap.find_opt key map with
    | Some lst -> 
      let new_lst = value :: lst in IntMap.add key new_lst map

    | None -> 
      let new_lst = [value] in IntMap.add key new_lst map 

  in
  List.fold_left (fun map x -> add_to_map x.order_id x map) IntMap.empty intermediate_results
 

let get_results inter_results = 
    
  let order_items = group_by inter_results 
  in
  let result_from_items order_items =
