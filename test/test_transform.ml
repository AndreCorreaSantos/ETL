...
let test_inner_join _ =
  let result = Etl.Transform.inner_join sample_items sample_orders in
...
let test_group_by _ =
  ...
  assert_bool "Empty list" (IntMap.is_empty (Etl.Transform.group_by []))

let test_get_results _ =
  let grouped : int_result list IntMap.t = Etl.Transform.group_by sample_int_results in
  let results : Etl.Types.result list = Etl.Transform.get_results grouped in
...
let test_group_by_ym _ =
  let result = Etl.Transform.group_by_ym sample_int_results in
  ...
  assert_bool "Empty list" (StringMap.is_empty (Etl.Transform.group_by_ym []))

let test_get_ym_results _ =
  let grouped = Etl.Transform.group_by_ym sample_int_results in
